...
delta=inputmatrix(:,1);
betaintensityvalues=inputmatrix(:,2);
%intensityvalues=inputmatrix(:,3);
lopticalfitting=inputmatrix(1,3); %best value of loptical determined by fitting cross section data
...
%%%value.
y = ((4.*pi.*hbar./(massSr./2).*amp.*lopticalfitting.*bohrradius.*(gammanatural./(delta.*1e9-deltae)).^2)-betaintensityvalues).^2./(betaintensityvalues.^2);