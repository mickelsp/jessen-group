...
%%% R4 2010.04.12 Pascal adds input for fitting error in case that there aren't enough points to calculate statistical uncertainty
...
function [xbuckets,ybuckets,yuncertainty] = buckets(xinput,yinput,yerror,bucketsize)
xvector=xinput(:);yvector=yinput(:);yerrorvector=yerror(:); %Define input vectors
    ...
    for b=1:numbuckets
        if (low(b)<=xvector(linecount)) && (xvector(linecount)<high(b))
            if counter1(b)==0
                ...
                bucketerrorsum(b)=yerrorvector(linecount);
            else
                bucketsum(b)=bucketsum(b)+yvector(linecount);
                bucketerrorsum(b)=(bucketerrorsum(b).^2+yerrorvector(linecount).^2).^0.5; %fitting error of each point in bucket; added in quadrature
            end
...
for b=1:numbuckets
    if counter1(b)==0
        ...
        bucketerror(counter2)=bucketerrorsum(b); %local temporary vector of error of values in bucket added in quadrature
...
%% Add statistical uncertainty to fit uncertainty in quadrature
totalerror=(sigmaerror(:).^2+bucketerror(:).^2).^0.5;
