...
%%% images of atoms and determines the lifetime of the atoms in the cloud.
...
batchfilename=char('20111205_LifetimeData');

batchfile=char(strcat(deblank(directory),deblank(batchfilename),deblank('.txt'))); %Create a text file with a list of filenames to be analyzed

[droptimevector TOFarea TOFtemperature]=textread(batchfile,'%f%f%f','commentstyle','matlab'); %Read in list of image file names
numberoffilenames=size(droptimevector,1);
...
fittingtype=2; %1 = Temperature fit; 2 = Lifetime fit; 3 = Linewidth fit
bodyloss=1; %1=fit to 1-body loss equation; 2=fit to 2-body loss equation
...
%% Data Preparation
xaxis=droptimevector;
Initialguess=[TOFarea(1,1),0.1]; %number, decay rate in s-1
%data=transpose([xaxis;TOFarea]);

%% Data Fitting
[coeff,r,J] = nlinfit(xaxis(:),TOFarea(:),@Lifetimefitfunction,Initialguess);
%%%Fitting results
ci=nlparci(coeff,r,J);
lifetime1body=1./coeff(2);
onebodydecay=coeff(2);
gammafitdev=(ci(2,2)-ci(2,1))./4;
onebodydecayerror=gammafitdev;
lifetime1bodyerror = onebodydecayerror./onebodydecay.^2;
indevariable=[0:2:max(xaxis)+20];
fitcurve=coeff(1).*exp(-coeff(2)*indevariable);

%%%Prepare data for fitting (two body decay rate)
% N0=ones(1,length(numatoms))*numatoms(1);  %in multiples of 10^6
% gamma=ones(1,length(numatoms))*coeff(2); %determine gamma from one body fit to final x # pts.  then, use as constant for 2-body fits
...
if bodyloss==2 %For 2-body losses in sample
    ...
    indevariable=[0:50:max(xaxis)];
    N2body=(initialnumber.*exp(-gammafit.*indevariable))./(1+((initialnumber.*betafit./gammafit).*(1-exp(-gammafit.*indevariable))));
...
%% Data Plotting
figure(1)
plot(xaxis,TOFarea,'sb','MarkerFaceColor','b','MarkerSize',markersize)
hold on
plot(indevariable,fitcurve,'-b','LineWidth',2);
...
ylabel('Area under TOF Curve [arb.]','FontSize',fontsize,'FontWeight','bold');
title('FORT Lifetime','FontSize',fontsize,'FontWeight','bold');
legend('TOF Area Measurement','1-Body Fit');
text(20,0.8,strcat('\tau \sim ',num2str(round(lifetime1body)),' \pm ',num2str(round(onebodydecayerror/onebodydecay^2)),' ms'),'FontSize',fontsize+4,'FontWeight','bold')
