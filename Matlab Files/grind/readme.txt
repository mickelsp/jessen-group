 +++++++++++++++++++++ GRIND for MATLAB ++++++++++++++++++++++++
 Version 1.0.8 (R2011a)   
 September 2010            
 tested on MATLAB R2011a will work on some earlier versions too
 ...
 GRIND for MATLAB
    ...
    - simulating sets of n ordinary differential equations, including time 
      delays and vector- and matrix notations. 
    - simulating sets of difference equations.
    - creating null-isoclines (nullclines) in phase spaces of 2 or 3 dimensions.
    - creating nullclines in spaces spanned by state variables and parameters. 
    - creating one-dimensional bifurcation plots by simulation.
    - stability analysis of equilibria, using eigenvalues of the Jacobian matrix.
    ...
    - various special analyses such as determining the Lyapunov coefficient
      to detect chaos, Poincaré sections etc.
    - discrete events within a continuous model
...
+++++++++++++++++++++ INSTALLING GRIND ++++++++++++++++++++++++
 ...
 Define model:
   ...
   loaddata     - Load datafile for parameter optimizing or external variables 
 ...
 Analysing:
   ...
   autocorr     - Autocorrelation function of state variables 
   ...
   locallyap    - Elner algoritm for local Lyapunov exponent
 ...
 Settings:
   ...
   enterjac     - Enter the derivatives used to calculate the Jacobian matrix 
   funcs        - Add/edit functions that can be used for axes and funplot 
   nextpen      - Assign the next color for g_grind.pen (the color of the next trajectory in the phase plane). 
   out          - Sets variables/functions for timeplots (at default all state variables) 
   par          - Show values of parameters 
   savemodel    - Save the model to an inifile 
   savepar      - Save model and current parameter settings to a file 
   setdata      - Enter data for parameter optimizing 
   setdefaults  - Save or get default settings 
   setpen       - Set style and color of runs in the phase plane (and dirfield) 
   setcolormap  - Set the color gradient in 3D plots 
