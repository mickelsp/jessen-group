...
%update g_permanent
if ~isempty(g_permanent.t) && g_permanent.active
   if nargin == 1 
    ...
    %  fprintf('%g   %g\n',g_permanent.lastt(g_permanent.lasti),at)
         ...
         if ~isempty(Y)
            for j = 1:length(g_grind.permanent)
               g_grind.permanent{j}.currvalue = Y(g_grind.permanent{j}.from:g_grind.permanent{j}.to);
               g_grind.permanent{j}.currvalue = reshape(g_grind.permanent{j}.currvalue, g_grind.permanent{j}.dims);
            end;
         end;
      ...
      for i = 1:length(g_grind.permanent)
         ...
         lcurr=length(curr);
         ato=g_grind.permanent{i}.to;
         afrom=g_grind.permanent{i}.from;
         if lcurr==0
            Y(g_grind.permanent{i}.from:g_grind.permanent{i}.to) = NaN;
         elseif (lcurr==1) || (lcurr == ato - afrom + 1)
            Y(afrom:ato) = curr;
         else
            size(curr)
            error('GRIND:defpermantent:NoSizeMatch','Size of permanent variable %s does not match', g_grind.permanent{i}.name);
         end;
      end;
