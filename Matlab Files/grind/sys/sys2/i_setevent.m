...
%   set(findobj(gcf,'Tag','EventsList'),'string',s);
 ...
 case 'check'
   if ~isempty(event)
      if isempty(which(event))
         ...
         errordlg(s);
         error('GRIND:setevent:FuncNotFound',s); %#ok
      end;
      ev.event = event;
 ...
 case 'list'
   ...
   for i = 1:length(g_grind.event.queue)
      ev = g_grind.event.queue(i);
      if length(ev.arg) == 1
         fprintf('Event %d: %s(%s) t=%s\n', i, ev.event, tostr(ev.arg{1}), ev.strt);
      elseif length(ev.arg) > 1
         fprintf('Event %d: %s(%s,%s) t=%s\n',i,ev.event,tostr(ev.arg{1}),tostr(ev.arg{2}),ev.strt);
      else
         fprintf('Event %d: %s t=%s\n', i, ev.event, ev.strt);
      end;
   end;
 case 'runnext'
   if nargin == 1
 ...
 case 'sortqueue'
   if ~isempty(g_grind.event.queue)
      ...
      [val, ix] = sort([g_grind.event.queue(:).t]);
 ...
 case 'enable'
   for i = 1:size(g_grind.event.events)
