function fig = i_plotdatadlg(flag)
if nargin == 0
   flag = 'init';
end;
switch flag
 ...
 case 'plot'
   TAB = sprintf('\t');
%   LF = sprintf('\n');
   h1=findobj(gcf,'tag','graphtype');
   plots = get(h1, 'string');
   graphtype = plots{get(h1, 'value')};
   h1=findobj(gcf,'tag','dataformat');
   forms = get(h1, 'string');
   dataformat = forms{get(h1, 'value')};
   h1=findobj(gcf,'tag','VarlistEdit');
   answer{1} = get(h1, 'String');
   h1=findobj(gcf,'tag','MatrixEdit');
   answer{2} = get(h1, 'String');
   l = length(answer{1}) + 1;
   l2 = l - 1;
   while l2 < l
      answer{1} = strrep(answer{1}, TAB, ' '); %#ok
      answer{1} = strrep(answer{1},'  ',' '); %#ok
      l = l2;
      l2 = length(answer{1});
   end;
   ...
   switch graphtype
    case 'scatterplot'
      if ~islineformat
         errordlg('Data type does not correspond with graph type');
   ...
   set(gca, 'TickDir', 'out');
   set(gca, 'Box', 'on');
   h1=findobj(gcbf,'tag','legendonoff');
   if (get(h1, 'value') == 1)
      if (length(varlist) > 2)
         xlabel(varlist{1});
 ...
 case 'load'
   ...
   if FileName ~= 0
      f = [PathName FileName];
      fillmat(tabread(f, 0), tabread(f, 1));
   end;
 case 'paste'
   fillmat(varpaste, varpaste('numeric'));
 case 'cancel'
   close;
end;

function fillmat(mat0, mat1)
mat = mat0;
vars = [];
if iscell(mat)
   vars = '';
...
if ~isempty(vars)
   h=findobj(gcbf,'tag','VarlistEdit');
   set(h, 'string', vars);
end;
s = num2str(mat);
h=findobj(gcbf,'tag','MatrixEdit');
set(h, 'string', s);
