function g_res1 = i_getoutfun(g_afun)
...
if (~isempty(g_data)) && strncmpi(g_afun, 'Observed ',9)
   ivar2 = i_getno(g_afun(10:length(g_afun)));
   g_res1 = g_data.obs(:, ivar2.no);
   return;
end;
...
if ~isempty(g_l_X.no) %shortcuts for simple output
   if g_l_X.ispar
      ...
      g_res1 = repmat(g_res1(:)',size(g_Y, 1), 1);
      return;
 %  elseif g_l_X.isfun
%       i_update_g_func;
...
[g_afun, g_afun2] = checkvecfuncs(g_afun);
g_l_symvar_afun = symvar(g_afun2);
%if ~g_grind.statevars.vector | (g_grind.statevars.dims{1}.dim2 == 1)
...
if ~isempty(g_grind) && isfield(g_grind, 'funcnames')&&~isempty(g_grind.funcnames.names) ...
    ...
    if g_grind.statevars.vector
      error(sprintf('Error: auxilary variables are no longer supported for output \nof vector/matrix models, define them instead \nas permanent variables using "defpermanent"'));%#ok
...
if ~isempty(g_l_i) && (length(g_l_i) == 1)
   ...
   for g_l_i = 1:size(g_Y, 1)
      NRes(g_l_i, :) = transpose(feval(g_grind.odefile, 1, g_N0));
   end;
   if ~g_grind.statevars.vector
   for g_l_i = 1:g_grind.statevars.dim
