...
%   $Revision: 1.13 $ $Date: 2002/04/08 20:26:51 $
...
c = isletter(s) | ((s >= '0') & (s <= '9')) | (s == '_'); %No convert
v = isletter(s);
...
d = find((s(1:end-1)=='.' & isletter(s(2:end)))); %No convert
if ~isempty(d)
  [b,ib] = setdiff1(b,d+1);
  e = e(ib);
end
...
if ~isempty(q)
  if rem(length(q),2)~=0,
    error('GRIND:model:SyntaxError','Quotes are not in pairs.')
  end
...
b = find(d==1);
e = find(d==-1)-1;
function [c,ia] = setdiff1(a, b)
%added as setdiff is not completely compatible with Octave
c=setdiff(a,b);
[aa,ia]=intersect(a,c);
