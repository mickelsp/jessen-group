function [g_l_ran,g_l_res] = i_funcalc(g_l_afun,g_l_avar,g_l_axrange,g_npoints,g_loc)
...
if isfield(g_grind, 'pars')
   if (~isempty(g_grind.pars))
      eval(i_globalstr([g_grind.pars {g_l_avar}]));
   end;
end;
...
if ~isempty(g_s_s11)
   ...
   if ~isempty(g_l_prompt)
      ...
      for g_l_i = 1:length(g_l_prompt)
         assignin('base', g_l_unknownpars{g_l_i}, str2num(Answer{g_l_i})); %#ok
      end;
   end;
end;
...
try
   ...
   g_l_s1=['g_l_res=' g_l_afun ';']; %#ok
   i_evalarray;
   if ~isempty(g_l_old)
      assignin('base', g_l_avar, g_l_old);
   end;
catch err
%   err=lasterror;
   if ~isempty(g_l_old)
